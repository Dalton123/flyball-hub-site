...
export type CustomUrl = {
  ...
  external?: string;
  isAffiliate?: boolean;
  affiliateProgram?: "awin" | "amazon" | "direct" | "other";
              ...
              markDefs: Array<
                | {
                    ...
                    affiliateProgram:
                      ...
                      | "other"
                      | null;
                    affiliateMerchant: string | null;
                    affiliatePlacementId: string | null;
                    href: string | "#" | null;
                  }
                | {
                    customLink?: CustomUrl;
        ...
        richText: Array<{
          ...
          markDefs: Array<
            | {
                ...
                openInNewTab: boolean | null;
                isAffiliate: boolean | null;
                affiliateProgram: "amazon" | "awin" | "direct" | "other" | null;
                affiliateMerchant: string | null;
                affiliatePlacementId: string | null;
                href: string | "#" | null;
              }
...
// Query: *[_type == "breed" && _id == $id][0]{      _id,  _type,  "title": select(    defined(ogTitle) => ogTitle,    defined(seoTitle) => seoTitle,    title  ),  "description": select(    defined(ogDescription) => ogDescription,    defined(seoDescription) => seoDescription,    description  ),  "image": coalesce(    seoImage.asset->url + "?w=860&h=860&fit=crop&auto=format&q=86",    image.asset->url + "?w=860&h=860&fit=crop&auto=format&q=86"  ),  "dominantColor": image.asset->metadata.palette.dominant.background,  "seoImage": seoImage.asset->url + "?w=1200&h=630&fit=fill&bg=fbf2d6&fm=jpg&q=85",  "logo": *[_type == "settings"][0].logo.asset->url + "?w=120&h=120&fit=max&auto=format&q=90",  "date": coalesce(date, publishedAt, _createdAt),    "title": select(      defined(ogTitle) => ogTitle,      defined(seoTitle) => seoTitle,      name + " - Flyball Breed Guide"    ),    "description": select(      defined(ogDescription) => ogDescription,      defined(seoDescription) => seoDescription,      defined(verdict) => verdict,      "Breed guide for flyball suitability, training fit and team role."    ),    name,    verdict,    verdictRating,    stats  }
